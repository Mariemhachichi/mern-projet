...
const Test = () => {
    const element = <FontAwesomeIcon icon={faTrashAlt} size="lg" color="#A52A2A"  />
    ...
    {Object.keys(usersData).filter(el=>usersData[el]).map((el,key) => (
      <article class="row fadeIn nfl" key={key}>
        <ul>
          <li className="metier">{usersData[el].metier}</li>
          <li><img src={usersData[el].picture} alt ="img" class="card__image" className="imgdshb" /></li>
          <li>{usersData[el].pseudo}</li>
          <li>{usersData[el].tel}</li>
          <li onClick={()=>deleteclientC(usersData[el]._id)}>{element}</li>
        </ul>
      </article>
      ))} 
    </section>   
